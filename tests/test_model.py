...
def test_model_1_3_1():
    ...
    assert np.isclose(
        model.get_layer(0).weights, np.asarray([[0.100875, -0.1, 0.2995]]), rtol=1e-1
    ).all()

    assert np.isclose(
        model.get_layer(1).weights,
        np.asarray([[0.700125], [0.9], [-0.399625]]),
        rtol=1e-1,
    ).all()
...
def test_model_1_1_1():
    ...
    assert np.isclose(model.get_layer(0).weights, np.asarray([[0.100255]])).all()

    assert np.isclose(model.get_layer(1).weights, np.asarray([[0.300085]])).all()
