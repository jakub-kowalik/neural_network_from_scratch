...
class Conv2DVectorized(Layer):
    ...
    def initialize_weights(self):
        if not (self.n_inputs is None or self.n_outputs is None):
            self.weights = 1e-3 * np.random.randn(
                self.n_outputs, self.n_inputs, self.filter_size, self.filter_size
            )
    ...
    def forward(self, x):
        ...
        windows = self.getWindows(
            x, (n, c, out_h, out_w), self.filter_size, self.padding, self.stride
        )
